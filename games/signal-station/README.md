...
本施工线只实现六个代表级，完整 96 级配置仍为 `HOLD`。从代表级迁移到未实现的相邻等级后，适配器会停止开启下一批并写入 `LEVEL_NOT_IMPLEMENTED` 覆盖阻断审计，而不是崩溃或静默套用近似配置。视觉素材仍是占位或历史参考，没有制作生产美术，也没有进行 Android 实机、临床环境或患者验证。
