...
- 固定 seed 生成链：UTF-8 FNV-1a32 → xorshift32 → 无模偏差拒绝采样 → Fisher–Yates；校验时从声明输入完整重放，拒绝“改内容后重算哈希”的伪造日程。
- 12 种水果、`FP_CORE_A`、`FP_CORE_B`、`FP_TRANSFER`、四种网格及 ≥56dp 命中区元数据；CORE_A 相似关系逐项绑定 v1.4 工作簿，CORE_B 明确标记为未获产品/美术批准，并携带必须由接入/发布门执行的机器可读 BLOCKED 声明。
...
- 代表级完整 Golden Vector、headless 结果证据、跨字段 Python 验证器；常规测试固定重放 1,800 份多 seed/多批次日程。
- 配置、日程、对象、批次、回调和正式结果均在边界处复制并递归冻结；会话只暴露不可变 `currentBatchView`，不泄漏可变批次运行对象。
- `BATCH_CLOSED` 采用“领域先提交、外部通知可重试”的有序 at-least-once 语义，重试不重算积分或迁级；会话与适配器入口均拒绝 sink 同步回入。
- L102 只在首次进入该等级后的首个正式教学批禁用第 1 波双光圈，后续批按 `MAX2_CONSEC` 正常生成。
...
- `HARDENING_REPORT.md`：第二轮对抗式复核发现、修复和证据。
- `REQUIREMENT_CONFLICTS.md`：冻结文本内部冲突及全量生产前待闭环项。
...
W2 没有导入 L1–L120 全量数值表，也没有实现 Cocos 场景、音频、生产动画和水果正式美术。缺失等级会直接拒绝，不会映射到最近代表级。8 张背景来自自包含工作包；水果路径仍是待正式资产线补齐的冻结引用。后续 W0 已提供 Android WebView 工程验证 HUD/触控并在 API 36 模拟器完成完整会话，但这不等于 Cocos、候选平板或性能验证。P45 同波水果唯一性与“固定目标 + 单波多个目标实例”的冲突仍采用最小一致解释，不能在未裁决前扩展为完整生产规则。
